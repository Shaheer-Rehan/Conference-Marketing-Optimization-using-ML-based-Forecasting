...
start_date = datetime.strptime(input("Enter the start date of the event in the format dd/mm/yyyy: "), "%d/%m/%Y") # Ask the user for the start date of the booking process
current_date = datetime.strptime(input("Enter the current date in the format dd/mm/yyyy: "), "%d/%m/%Y") # Ask the user for the current date
event_date = datetime.strptime(input("Enter the event date in the format dd/mm/yyyy: "), "%d/%m/%Y") - timedelta(days = 1) # Ask the user for the event date
date_range = pd.date_range(start = start_date, end = event_date) # Create a date range from the start date to the event date
input_df = pd.DataFrame(date_range, columns = ["Date"]) # Create a new dataframe with the date range
input_df["Days to Event"] = (pd.to_datetime(event_date, dayfirst = True) - input_df["Date"]).dt.days # Add a new column to the dataframe showing the days left to the event
input_df["Day #"] = (input_df["Days to Event"].max() + 1) - input_df["Days to Event"] # Reverse the order of the days to event column to start from 1 for the model to work properly
input_df["Advertisement Status"] = input_df["Days to Event"].apply(lambda x: 0) # Add a new column to the dataframe to indicate advertisement status. Assuming that no ads were launched before current date
current_booking_count = int(input("Enter the booking count so far: ")) # Ask the user for the booking count upto the current date
ad_choice = input("Would you like to launch an advertisement? (yes/no): ") # Ask the user if they would like to launch an advertisement
...
train_x_final, train_y_final, l1_ratio, alpha, ad_effect = elastic_net_data(df_ITM_days) 
target_days_df = input_df[input_df["Date"] >= current_date].copy() # Create a new dataframe with the days from the current date to the event date
...
pred_final = regr_final.predict(target_days_df[["Day #", "Advertisement Status"]]) # Make predictions using the testing dataset
target_days_df.loc[:, "Predicted Count"] = pred_final # Add a new column for the predicted booking count
if ad_choice.lower() == "yes":
    target_days_df.loc[target_days_df.index[:7], "Predicted Count"] *= (1 + ad_effect) # Update the ad status to 1 for the next 7 days 
total_event_bookings = int(round(current_booking_count + target_days_df["Predicted Count"].sum(), 0)) # Calculate the total predicted bookings for the event
